namespace StockStudy
{
    ...
    {
        public static decimal StandardDeviation(this IEnumerable<decimal> priceList, decimal ema)
        {
            // 各个软件中BOLL指标误差较大 两位小数
            if (!priceList.Any()) return 0; // 如果没有数据，返回0
            
            var sqrt = Math.Sqrt(Convert.ToDouble(priceList.Select(price => price - ema).Select(e => e * e).Sum()) / Convert.ToDouble(priceList.Count()));
